...
// ============================================================
//  Service worker : SUPPRIMÉ (plus de fichier sw.js).
...
//  Au chargement, on désinscrit directement tout SW résiduel (sans dépendre d'un
//  fichier kill-switch) et on vide les caches. Sur un client neuf : aucun SW.
// ============================================================
let _swReg = null;   // conservé pour compat ; toujours null désormais
if ("serviceWorker" in navigator) {
  navigator.serviceWorker.getRegistrations()
    .then(regs => regs.forEach(r => r.unregister().catch(() => {})))  // purge l'ancien SW
    .catch(() => {});
}
...
async function loadMyGames() {
  ...
  const { modeDisplayName } = await import("./scrabble/engine.js?v=347");
  ...
  // Restaurer le sous-onglet mémorisé (Tournoi / Entraînement) après reload.
  let gtab = "tournoi";
  try { gtab = localStorage.getItem("topissimo:gtab") || "tournoi"; } catch (_) {}
  switchGamesTab(gtab);
}
...
window.switchGamesTab = function(which) {
  ...
  try { localStorage.setItem("topissimo:gtab", which); } catch (_) {}
};
...
window.toggleFfscFav = async function(id, ev) {
  ...
  let favs = ffscFavs();
  const existing = favs.find(t => t.id === id);
  if (existing) {
    // Suppression DÉFINITIVE (et non un simple repli) → on confirme pour éviter
    // les clics accidentels (le joueur veut souvent juste replier le tournoi).
    if (!confirm(`Retirer « ${existing.name} » de Mes tournois ?\n(Pour seulement replier le tournoi, clique sur sa ligne.)`)) return;
...
function renderFfscFavs() {
  ...
  const item = (t) => `
    <details class="fav-row" data-id="${t.id}" data-name="${escapeHtml(t.name)}" ontoggle="onFavToggle(this)" style="border:1px solid var(--soft);border-radius:8px">
      <summary style="cursor:pointer;display:flex;align-items:center;gap:6px;padding:4px 10px;list-style:none">
        <span style="flex:1">${escapeHtml(t.name)}${t.date ? ` <span class="muted">(${t.date})</span>` : t.year ? ` <span class="muted">(${t.year})</span>` : ""}</span>
        <button class="fav-remove" title="Retirer ce tournoi de Mes tournois" onclick="event.preventDefault();event.stopPropagation();toggleFfscFav('${t.id}', event)">🗑</button>
      </summary>
      <div class="fav-parties" style="padding:0 10px 8px"></div>
    </details>`;
  // Chaque SAISON est elle-même un accordéon : on déplie en cliquant sur le
  // millésime. La plus récente est ouverte par défaut, les autres repliées
  // (liste compacte, plus besoin de scroller pour atteindre les saisons du bas).
  list.innerHTML = saisons.map((s, i) => `
    <details class="fav-saison" ${i === 0 ? "open" : ""}>
      <summary>${s} <span class="muted" style="font-weight:400">(${bySaison[s].length})</span></summary>
...
function expandFav(id) {
  ...
  if (d) {
    d.closest(".fav-saison")?.setAttribute("open", "");   // ouvre la saison parente
    d.open = true;
...
window.openFfscRelier = async function(fisfId) {
  ...
  $("#ffscRelierSub").textContent = `${t.saison} · joué le ${t.date} — tournois FFSC du même jour ⭐. Pour les interclubs, relie chaque ligne (P1, P2…) au même tournoi : elles seront regroupées.`;
  $("#ffscRelierModal").hidden = false;
  const _link = $("#ffscRelierLink"); if (_link) _link.value = "";
  const _ls = $("#ffscRelierLinkStatus"); if (_ls) _ls.textContent = "";
...
window.renderFfscRelierCandidates = function() {
  ...
  if (!t) return;
  // On ne propose QUE les tournois FFSC du même jour (⭐). Les autres jours ne sont
  // jamais le bon tournoi et noyaient la liste ; pour les cas absents du calendrier
  // daté (Championnats du monde, id string…), on passe par « Recherche avec l'ID ».
  const items = all.filter(x => x.date === t.date)
    .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
  const list = $("#ffscRelierList");
  if (!items.length) {
    list.innerHTML = `<p class="muted">Aucun tournoi FFSC le ${t.date}. Utilise « Recherche avec l'ID du tournoi » ci-dessous.</p>`;
    return;
  }
  list.innerHTML = items.map(x => `
    <button class="btn ghost small" style="text-align:left"
      onclick="confirmFfscRelier(${x.id})">
      ⭐ ${escapeHtml(x.name)} <span class="muted">${x.date || ""}</span>
    </button>`).join("");
};
...
window.renameCurrentTournament = async () => {
  ...
  if (!trimmed || trimmed === current) return;     // vide ou inchangé
  const { error } = await sb.from("tournaments").update({ name: trimmed }).eq("id", currentTournamentId);
  if (error) { alert("Renommage : " + error.message); return; }
  $("#tournamentDetailTitle").textContent = `🏟 ${trimmed}`;
  loadTournaments();                               // rafraîchit la liste en arrière-plan
};
    ...
    <div style="margin-bottom:16px">
      ...
      <div style="display:flex;flex-direction:column;gap:7px">${progList.map(p => `
        <div style="display:flex;align-items:center;gap:10px;font-size:.9rem">
          <span style="flex:0 0 150px;font-weight:600;white-space:nowrap;text-align:right">${escapeHtml(p.name)}</span>
...
window.showPodiumSheet = function(playerId) {
  ...
  $("#podiumSheetBody").innerHTML = `<div class="table-wrap"><table style="width:100%;border-collapse:collapse;font-size:.85rem;white-space:nowrap">
    <thead><tr style="background:var(--petrol);color:#fff">
      ...
      <th style="padding:4px 6px;text-align:center">Négatif</th>
...
async function loadTournamentLeaderboard(tournamentId, games, tournamentName, modeDisplayName) {
  ...
  // onlyCat : dans un sous-classement (Blitz, Originales…), on ne détaille que
  // les parties de CETTE catégorie ; dans le général, on affiche tout.
  const expandHtml = (p, onlyCat = null) => {
    const catsToShow = onlyCat ? [onlyCat] : orderedCats;
    let html = `<div class="expand-inner"><table>
      <thead><tr><th>Catégorie</th><th>Partie</th><th>Négatif</th><th>Loupés</th><th>Temps</th><th></th></tr></thead><tbody>`;
    for (const c of catsToShow) cats[c].forEach((g) => {
      const r = p.perGame[g.id];
      // Picto 📱 si la partie a été jouée sur mobile — ferré à GAUCHE d'une cellule
      // de largeur fixe, le chrono ferré à droite : ainsi les pictos s'alignent
      // entre eux et les chronos aussi (bords droits alignés), avec un écart réduit.
      ...
      html += `<tr><td>${cat.labelOf(c)}</td><td>${escapeHtml(g.name)}</td><td class="neg">${r ? r.neg : "—"}</td><td>${r ? r.missed : "—"}</td><td style="white-space:nowrap"><span style="display:inline-flex;justify-content:space-between;width:5.5em"><span>${mobileIcon}</span><span>${r ? fmtT(r.time) : "—"}</span></span></td><td style="text-align:right">${sheetBtn}</td></tr>`;
    });
  ...
  const catRowFn = (c) => (p) => {
    ...
    return `<tr class="${p.id === me ? 'me' : ''}" onclick="toggleLbRow(this)" data-pid="${p.id}">
      ...
      <tr class="expand-row" hidden><td colspan="4">${expandHtml(p, c)}</td></tr>`;
  };
...
async function loadTournamentStats(tournamentId, games) {
  ...
  const { emptyBoard, applyMove, GAME_MODES } = await import("./scrabble/engine.js?v=347");
  const gameById = {};
...
const PG_DEFAULT_TIME = {
  ...
  horizvert: 120, topsoustop: 180, infjoker: 120, grillerandom: 120, sablier: 120, gigogne: 120,
};
  ...
  try {
    ...
    try {
      mods = await Promise.all([
        import("./scrabble/dictionary.js?v=347"),
        import("./scrabble/generator.js?v=347"),
        import("./scrabble/engine.js?v=347"),
      ]);
    } catch (e) {
...
window.recomputeAllNeg = async function(force = false) {
  ...
  let recomputeResult;
  try {
    ({ recomputeResult } = await import("./scrabble/recompute.js?v=347"));
  } catch (e) {
...
async function onSignedIn() {
  ...
  localStorage.setItem("currentPseudo", player.name || "");   // utilisé par la page de jeu (gating admin)
...
function onSignedOut() {
  ...
  localStorage.removeItem("currentPseudo");
...
window.openPlayerGameSheet = function(playerId, gameId) {
  ...
  const rows = r.details.map(h => {
    // Coup raté = top non trouvé (négatif < 0) ou abandon / temps écoulé.
