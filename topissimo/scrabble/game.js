...
} from "./engine.js?v=366";
import { Dictionary } from "./dictionary.js?v=366";
import { findTop, findTopRanked, rankIsotops, snakeBestTop, snakeMoveLegal } from "./topfinder.js?v=366";
...
const BUILD_VERSION = "garenna-v366";
...
function renderPace() {
  ...
  const build = (key, mineVal, fmt, dir) => {
    ...
    const html = rows.map(r =>
      ...
      + `<span class="v">${fmt(r.val)}</span></div>`).join("");
...
function renderReviewSolutions(idx) {
  ...
  setTimeout(() => {
    ...
    if (review.active && state.settings.withJoker && !review.replayMode) {
